...
def main():
    ...
    photos = photosdb.photos(albums=['Imports_Album'])
    ...
    for p in photos:
        ...
        # The folder structure - right now it is root / year / month
            ...
            # else:
            fold_struc = '/Volumes/YOGI/tmp/'
            fold_struc_mov = '/Volumes/YOGI//'
            fold_struc_no_model='/Volumes/YOGI/tmp/none/'

            prep_dir = '/Volumes/YOGI/tmp/tmp/'
            ...
            if new_name.lower().endswith('.heic'):
                ...
                new_name_prefix = os.path.splitext(new_name)[0]
                new_name_fix = prep_dir + new_name, prep_dir+ new_name_prefix + '.jpg'
                old_heic = prep_dir + new_name
                new_jpg = prep_dir + str(new_name_fix)
                # osxphotos.fileutil.FileUtilMacOS.convert_to_jpeg('/Volumes/YOGI/tmp/tmp/20210403_IMG_0045.HEIC','/Volumes/YOGI/tmp/tmp/20210417_IMG_0045.jpg',1.0)
